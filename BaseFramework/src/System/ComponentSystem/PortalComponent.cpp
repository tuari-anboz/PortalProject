							...
							if (dot < 0)
							{
								...
								if (cam)
								{
									...
									if (fpscomp)
									{
										...
										GameMgr.m_Editor_Log.AddLog("%f", fpscomp->GetAngX());
										fpscomp->GetAngX() = ownerdot;

										GameMgr.m_Editor_Log.AddLog("%f", ownerdot);
...
// _posはあたった場所
// _normalはあたったポリゴンの法線
void PortalComponent::Set(const KdVec3& _raypos, const KdVec3& _pos, const KdVec3& _normal, const int _no)
{
	auto owner = GetOwner();
